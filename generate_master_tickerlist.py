...
def load_manual_tickers():
    ...
    try:
        manual_file = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "tickerlist.txt"
        )
...
def generate_master_tickerlist():
    ...
    try:
        ...
        logger.info(
            f"🎯 Master ticker list generated: {len(master_tickers)} total tickers (manual only)"
        )
        ...
        output_file = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "master_tickerlist.csv"
        )
        ...
        if upload_success:
            ...
            logger.warning(
                f"⚠️ Failed to upload to Spaces, but local file created: {output_file}"
            )
    ...
    try:
        ...
        if success:
            ...
            update_scheduler_status(
                job_name, "Fail", "Failed to generate master ticker list"
            )
