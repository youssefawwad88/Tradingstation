...
from utils.helpers import read_master_tickerlist, save_df_to_s3, update_scheduler_status, load_manual_tickers
...
def run_full_rebuild():
    ...
    - Follows exact specifications: Daily (200 rows), 30min (500 rows), 1min (past 7 days)
    """
    ...
    for ticker in tickers:
        ...
        # 3. 1-Minute Intraday Data (past 7 days, excluding today's 9:30–9:45 for volume averages)
        try:
            ...
            if not intraday_1min_df.empty:
                ...
                # Exclude today's 9:30–9:45 data to avoid bias in volume averages
                today = datetime.now().date()
                exclude_start = pd.Timestamp.combine(today, pd.Timestamp('09:30:00').time())
                exclude_end = pd.Timestamp.combine(today, pd.Timestamp('09:45:00').time())
                
                # Filter out the exclusion window
                intraday_1min_df = intraday_1min_df[
                    ~((intraday_1min_df['timestamp'] >= exclude_start) & 
                      (intraday_1min_df['timestamp'] <= exclude_end))
                ]
                
                upload_success = save_df_to_s3(intraday_1min_df, f'data/intraday/{ticker}_1min.csv')
                if upload_success:
                    logger.debug(f"Saved {len(intraday_1min_df)} rows of 1-min data for {ticker} (last 7 days, excluding today's 9:30-9:45)")
                else:
