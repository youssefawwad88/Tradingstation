...
def convert_global_quote_to_dataframe(quote_data, ticker):
    ...
    try:
        ...
        df = pd.DataFrame(
            {
                "timestamp": [
                    current_minute
                ],  # Keep as timezone-aware datetime for proper comparison
                "open": [
                    quote_data["price"]
                ],  # For real-time: open=current price (will be updated intelligently)
                "high": [
                    quote_data["price"]
                ],  # For real-time: high=current price (will be updated intelligently)
                "low": [
                    quote_data["price"]
                ],  # For real-time: low=current price (will be updated intelligently)
        ...
        logger.debug(
            f"Converted GLOBAL_QUOTE to DataFrame for {ticker}: {len(df)} row with timezone-aware timestamp {current_minute}"
        )
        return df

    except Exception as e:
        logger.error(
            f"Error converting GLOBAL_QUOTE data to DataFrame for {ticker}: {e}"
        )
        return pd.DataFrame()
...
def standardize_timestamps(df, data_type):
    ...
    try:
        ...
        if df[timestamp_col].dtype == "datetime64[ns, America/New_York]" or (
            hasattr(df[timestamp_col].iloc[0], "tzinfo")
            and df[timestamp_col].iloc[0].tzinfo is not None
        ):
            logger.debug(
                f"Timestamps already timezone-aware, converting to UTC for storage"
            )
            df_copy = df.copy()
            # Convert to UTC and format as strings
            df_copy[timestamp_col] = df_copy[timestamp_col].dt.tz_convert("UTC")
            df_copy[timestamp_col] = df_copy[timestamp_col].dt.strftime(
                "%Y-%m-%d %H:%M:%S+00:00"
            )
            return df_copy
        else:
            # Apply the centralized timestamp standardization for non-timezone-aware data
            standardized_df = apply_timestamp_standardization_to_api_data(
                df, data_type=data_type
            )
            logger.debug(
                f"Timestamp standardization applied for {data_type} data: {len(standardized_df)} rows"
            )
...
def intelligent_append_or_update(existing_df, new_df):
    ...
    if existing_df.empty:
        ...
        logger.info(
            f"   Original: 0 rows, New: {len(new_df)} rows, Final: {len(new_df)} rows"
        )
        return new_df

    if new_df.empty:
        logger.info(
            "📊 Intelligent Append: No new data - returning existing data unchanged"
        )
        logger.info(
            f"   Original: {len(existing_df)} rows, New: 0 rows, Final: {len(existing_df)} rows"
        )
        return existing_df
    ...
    try:
        ...
        if timestamp_col not in existing_df.columns:
            logger.error(
                f"Timestamp column '{timestamp_col}' not found in existing data"
            )
            return existing_df
        ...
        # BULLETPROOF TIMESTAMP COMPARISON as per problem statement requirements
        # Implement explicit, multi-step process that standardizes all timestamps to America/New_York timezone

        # Step 1: Convert History to Datetime - Convert timestamp column to timezone-aware datetime objects
        logger.debug("🕐 Step 1: Converting history timestamps to datetime objects")
        ...
        # Ensure CSV data (stored in UTC) becomes UTC-aware datetime objects
        if len(existing_df) > 0:
            if existing_df[timestamp_col].dt.tz is None:
                # CSV data is naive, localize as UTC (since CSV is stored in UTC format)
                logger.debug(
                    "   History: Localizing naive timestamps as UTC (CSV standard)"
                )
                existing_df[timestamp_col] = existing_df[timestamp_col].dt.tz_localize(
                    "UTC"
                )
            else:
                # Already timezone-aware, convert to UTC if not already
                logger.debug(
                    f"   History: Converting from {existing_df[timestamp_col].dt.tz} to UTC"
                )
                existing_df[timestamp_col] = existing_df[timestamp_col].dt.tz_convert(
                    "UTC"
                )

        if len(new_df) > 0:
            if new_df[timestamp_col].dt.tz is None:
                # Live data should be in NY timezone, but if naive, localize as UTC for consistency
                logger.debug("   Live: Localizing naive timestamps as UTC")
                new_df[timestamp_col] = new_df[timestamp_col].dt.tz_localize("UTC")
            else:
                # Already timezone-aware, convert to UTC
                logger.debug(
                    f"   Live: Converting from {new_df[timestamp_col].dt.tz} to UTC"
                )
                new_df[timestamp_col] = new_df[timestamp_col].dt.tz_convert("UTC")

        if len(existing_df) > 0 and len(new_df) > 0:
            # Step 2: Get Last Historical Time (UTC) - Get the last timestamp from history as UTC
            logger.debug("🕐 Step 2: Getting last historical timestamp (UTC)")
            existing_df_sorted = existing_df.sort_values(timestamp_col)
            last_historical_ts_utc = existing_df_sorted.iloc[-1][timestamp_col]
            logger.debug(
                f"   Last historical timestamp (UTC): {last_historical_ts_utc}"
            )

            # Step 3: Get Live Time (UTC) - Get current live timestamp as UTC
            logger.debug("🕐 Step 3: Getting live timestamp (UTC)")
            live_ts_utc = new_df.iloc[0][timestamp_col]
            logger.debug(f"   Live timestamp (UTC): {live_ts_utc}")

            # Step 4: Convert BOTH to New York Time - Create timezone-aware NY variables
            logger.debug(
                "🕐 Step 4: Converting both timestamps to America/New_York timezone"
            )
            ny_tz = pytz.timezone("America/New_York")

            last_historical_ts_nyc = last_historical_ts_utc.tz_convert(ny_tz)
            live_ts_nyc = live_ts_utc.tz_convert(ny_tz)

            logger.debug(
                f"   Last historical timestamp (NYC): {last_historical_ts_nyc}"
            )
            logger.debug(f"   Live timestamp (NYC): {live_ts_nyc}")

            # Step 5: Perform Comparison (in NY Time) - Compare minute/hour on timezone-aware _nyc variables
            logger.debug(
                "🕐 Step 5: Performing comparison in America/New_York timezone"
            )

            # Truncate to minute precision for comparison (remove seconds and microseconds)
            last_historical_minute_nyc = last_historical_ts_nyc.replace(
                second=0, microsecond=0
            )
            live_minute_nyc = live_ts_nyc.replace(second=0, microsecond=0)

            logger.info(f"📊 Bulletproof Timestamp Comparison:")
            logger.info(
                f"   Last historical minute (NYC): {last_historical_minute_nyc}"
            )
            logger.info(f"   Live quote minute (NYC): {live_minute_nyc}")

            # Convert existing timestamps back to NY timezone for processing
            existing_df_sorted[timestamp_col] = existing_df_sorted[
                timestamp_col
            ].dt.tz_convert(ny_tz)
        ...
        if len(new_df) != 1:
            logger.warning(
                f"Expected 1 row in new real-time data, got {len(new_df)} rows"
            )
            return existing_df
        ...
        if len(existing_df) > 0:
            # CORE LOGIC: Compare timestamps using bulletproof NYC comparison variables
            if live_minute_nyc == last_historical_minute_nyc:
                ...
                existing_df_sorted.loc[last_idx, "close"] = (
                    new_close  # Always update close to current price
                )
                # Note: open stays the same, volume could be updated but we'll keep existing

                logger.info(
                    f"   Updated candle: high={updated_high}, low={updated_low}, close={new_close}"
                )
                logger.info(
                    f"   Original: {len(existing_df)} rows, Updated: 0 new rows, Final: {len(existing_df_sorted)} rows"
                )
            ...
            elif live_minute_nyc > last_historical_minute_nyc:
                ...
                logger.info(
                    f"   Appended new candle: timestamp={new_timestamp}, price={new_close}"
                )
                logger.info(
                    f"   Original: {len(existing_df)} rows, New: 1 candle, Final: {len(combined_df)} rows"
                )

                # Convert back to UTC for storage (as required by the system)
                combined_df[timestamp_col] = combined_df[timestamp_col].dt.tz_convert(
                    "UTC"
                )
                combined_df[timestamp_col] = combined_df[timestamp_col].dt.strftime(
                    "%Y-%m-%d %H:%M:%S+00:00"
                )
                ...
                logger.warning(
                    f"⚠️ New timestamp {live_minute_nyc} is older than last candle {last_historical_minute_nyc}"
                )
            ...
            new_df[timestamp_col] = new_df[timestamp_col].dt.tz_convert("UTC")
            new_df[timestamp_col] = new_df[timestamp_col].dt.strftime(
                "%Y-%m-%d %H:%M:%S+00:00"
            )
        ...
        logger.warning(
            f"🔄 Returning existing data as fallback to avoid data loss ({len(existing_df)} rows)"
        )
        return existing_df
...
def resample_1min_to_30min(df_1min):
    ...
    try:
        ...
        if timestamp_col not in df_1min.columns:
            logger.error(
                f"Timestamp column '{timestamp_col}' not found in 1-minute data"
            )
            return pd.DataFrame()
        ...
        logger.info(
            f"📊 Resampled 1-minute data: {len(df_1min)} rows → 30-minute data: {len(df_30min)} rows (trimmed to 500)"
        )
    ...
    try:
        ...
        try:
            ...
            if existing_1min_df.empty:
                logger.warning(
                    f"⚠️ {ticker}_1min.csv not found or is incomplete. Triggering a full data fetch to repair."
                )
                data_is_healthy = False
            else:
        ...
        if data_is_healthy and existing_1min_df is not None:
            try:
                ...
                if os.path.exists(local_file_path):
                    ...
                    logger.debug(
                        f"Local file size for {ticker}: {file_size_bytes} bytes"
                    )
                else:
                    ...
                    estimated_size = (
                        len(existing_1min_df) * 60
                    )  # Realistic estimate based on testing
                    file_size_bytes = estimated_size
                    logger.debug(
                        f"Estimated file size for {ticker}: {file_size_bytes} bytes (from {len(existing_1min_df)} rows)"
                    )
                ...
                if file_size_bytes <= min_file_size:
                    ...
                    logger.debug(
                        f"Health check failed for {ticker}: insufficient file size ({file_size_bytes} bytes, minimum {min_file_size} bytes required)"
                    )
                    data_is_healthy = False
                ...
                if data_is_healthy:
                    ...
                    missing_columns = [
                        ...
                        if col not in existing_1min_df.columns
                    ]
                    if missing_columns:
                        logger.warning(
                            f"⚠️ {ticker}_1min.csv not found or is incomplete. Triggering a full data fetch to repair."
                        ...
                        logger.debug(
                            f"Health check failed for {ticker}: missing columns {missing_columns}"
                        )
                        data_is_healthy = False
                ...
                if data_is_healthy:
                    ...
                    if (
                        len(existing_1min_df) < 1000
                    ):  # Conservative minimum for multi-day history
                        logger.warning(
                            f"⚠️ {ticker}_1min.csv not found or is incomplete. Triggering a full data fetch to repair."
                        )
                        logger.debug(
                            f"Health check failed for {ticker}: insufficient history ({len(existing_1min_df)} rows, expected substantial 7-day history)"
                        )
                        data_is_healthy = False

                if data_is_healthy:
                    logger.debug(
                        f"✅ Health check passed for {ticker}: {len(existing_1min_df)} rows, {file_size_bytes} bytes with required columns"
                    )
                    return True
                ...
                logger.debug(
                    f"Health check integrity validation failed for {ticker}: {e}"
                )
                data_is_healthy = False
        ...
        if not data_is_healthy:
            logger.info(
                f"🔧 Self-Healing: Initiating full data fetch to repair {ticker}..."
            )
            try:
                ...
                if save_results["saves_successful"] > 0:
                    logger.info(
                        f"✅ Self-Healing: Successfully repaired data for {ticker} ({save_results['saves_successful']}/{save_results['saves_attempted']} files saved)"
                    )
                    ...
                    try:
                        ...
                        if not repaired_df.empty and len(repaired_df) >= 1000:
                            logger.info(
                                f"✅ Self-Healing: Repair verification successful for {ticker} - {len(repaired_df)} rows available"
                            )
                            return True
                        else:
                            logger.error(
                                f"❌ Self-Healing: Repair verification failed for {ticker} - data still insufficient after repair"
                            )
                            return False
                    except Exception as e:
                        logger.error(
                            f"❌ Self-Healing: Repair verification failed for {ticker}: {e}"
                        )
                        return False
                else:
                    logger.error(
                        f"❌ Self-Healing: Failed to repair data for {ticker} (no files saved successfully)"
                    )
                    return False

            except Exception as e:
                logger.error(
                    f"❌ Self-Healing: Full data fetch failed for {ticker}: {e}"
                )
                return False
        ...
        logger.warning(
            f"⚠️ {ticker}_1min.csv not found or is incomplete. A full data fetch is required for this ticker."
        )
...
def process_ticker_realtime(ticker):
    ...
    try:
        ...
        if not check_ticker_data_health(ticker):
            ...
            logger.info(
                f"⏭️ Skipping real-time update for {ticker} due to missing/incomplete data"
            )
            return (
                True  # Return True to continue processing other tickers (graceful skip)
            )

        logger.debug(
            f"✅ Step A passed: {ticker} has healthy data, proceeding with real-time update"
        )
        ...
        try:
            ...
            if not quote_data:
                ...
                logger.warning(
                    f"⚠️ No real-time data received for {ticker} - API may have failed or market closed"
                )
                return (
                    True  # Consider this a success since it's not a processing failure
                )

            logging.info(f"[{ticker}] Real-time quote received.")
            logger.info(
                f"📥 Received real-time quote for {ticker}: price=${quote_data['price']}"
            )

        except Exception as e:
            logger.error(
                f"❌ STEP 1 FAILED - Error fetching GLOBAL_QUOTE for {ticker}: {e}"
            )
        ...
        try:
            logger.debug(
                f"🔄 Step 2: Converting GLOBAL_QUOTE to DataFrame for {ticker}..."
            )
            new_df = convert_global_quote_to_dataframe(quote_data, ticker)

            if new_df.empty:
                logger.error(
                    f"❌ STEP 2 FAILED - Failed to convert GLOBAL_QUOTE data to DataFrame for {ticker}"
                )
                logger.error(
                    f"❌ {ticker}: Data transformation failed during DataFrame conversion step"
                )
                return False

            logger.debug(
                f"✅ Real-time data converted to DataFrame for {ticker}: {len(new_df)} row"
            )

        except Exception as e:
            logger.error(
                f"❌ STEP 2 FAILED - Error converting GLOBAL_QUOTE to DataFrame for {ticker}: {e}"
            )
            logger.error(
                f"❌ {ticker}: Data transformation failed during DataFrame conversion step"
            )
        ...
        try:
            ...
            logger.error(
                f"❌ STEP 3 FAILED - Error standardizing timestamps for {ticker}: {e}"
            )
            logger.error(
                f"❌ {ticker}: Timestamp processing failed during standardization step"
            )
            return False
        ...
        try:
            ...
            logging.info(
                f"[{ticker}] Loaded existing 1min file... It has {existing_count} rows."
            )
            logger.debug(f"📊 Existing 1min data for {ticker}: {existing_count} rows")

        except Exception as e:
            logger.error(
                f"❌ STEP 4 FAILED - Error loading existing 1min data for {ticker}: {e}"
            )
        ...
        try:
            logger.debug(
                f"🧠 Step 5: Applying intelligent append/update logic for {ticker}..."
            )
            updated_1min_df = intelligent_append_or_update(existing_1min_df, new_df)
            logging.info(
                f"[{ticker}] Intelligent update complete. 1min DataFrame now has {len(updated_1min_df)} rows."
            )
            ...
            if rows_changed > 0:
                logging.info(
                    f"[{ticker}] 1-min data changed: +{rows_changed} rows. Saving..."
                )
                logger.info(
                    f"INFO: Intelligent update successful: Added {rows_changed} new candles for {ticker} (1-min)"
                )
            elif rows_changed == 0 and final_1min_count > 0:
                logging.info(
                    f"[{ticker}] 1-min data updated in-place (same minute). Saving..."
                )
                logger.info(
                    f"INFO: Intelligent update successful: Updated existing candle for {ticker} (1-min)"
                )
            else:
                logging.info(
                    f"[{ticker}] No 1-min data changes detected. Skipping saves."
                )
                return True

        except Exception as e:
            logger.error(
                f"❌ STEP 5 FAILED - Error during intelligent append/update for {ticker}: {e}"
            )
            logger.error(
                f"❌ {ticker}: Data merge failed during intelligent append/update step"
            )
            return False
        ...
        try:
            ...
            if not save_1min_success:
                logger.error(
                    f"❌ STEP 6 FAILED - Failed to save 1-minute data for {ticker}"
                )
                logger.error(
                    f"❌ {ticker}: File save failed during 1min data save step"
                )
                return False

            logger.info(
                f"✅ Successfully saved 1-minute data for {ticker}: {final_1min_count} rows"
            )

        except Exception as e:
            logger.error(
                f"❌ STEP 6 FAILED - Error saving 1-minute data for {ticker}: {e}"
            )
        ...
        try:
            logger.debug(
                f"📊 Step 7: Resampling 1-minute to 30-minute data for {ticker}..."
            )
            resampled_30min_df = resample_1min_to_30min(updated_1min_df)

            if resampled_30min_df.empty:
                logger.warning(
                    f"⚠️ STEP 7 WARNING - Failed to resample 30-minute data for {ticker}"
                )
                logger.warning(
                    f"⚠️ {ticker}: Resampling failed during 30min data creation step, but 1min save succeeded"
                )
                return True  # 1-min save was successful, so this is still a partial success

            logger.info(
                f"📊 Resampled 30-minute data for {ticker}: {len(resampled_30min_df)} rows"
            )

        except Exception as e:
            logger.error(
                f"❌ STEP 7 FAILED - Error resampling 30-minute data for {ticker}: {e}"
            )
            logger.warning(
                f"⚠️ {ticker}: Resampling failed during 30min data creation step, but 1min save succeeded"
            )
        ...
        try:
            ...
            if save_30min_success:
                logger.info(
                    f"✅ Successfully saved 30-minute data for {ticker}: {len(resampled_30min_df)} rows"
                )
                logger.info(
                    f"🎉 Complete real-time update success for {ticker}: 1-min ✅, 30-min ✅"
                )
                return True
            else:
                logger.warning(
                    f"⚠️ STEP 8 WARNING - Failed to save 30-minute data for {ticker}, but 1-minute save succeeded"
                )
                logger.warning(
                    f"⚠️ {ticker}: File save failed during 30min data save step, but 1min save succeeded"
                )
                return True  # Partial success is still success

        except Exception as e:
            logger.error(
                f"❌ STEP 8 FAILED - Error saving 30-minute data for {ticker}: {e}"
            )
            logger.warning(
                f"⚠️ {ticker}: File save failed during 30min data save step, but 1min save succeeded"
            )
        ...
        logger.error(
            f"❌ CRITICAL FAILURE - Unexpected error processing real-time data for {ticker}: {e}"
        )
        logger.error(
            f"❌ {ticker}: Critical failure in real-time processing - continuing with next ticker"
        )
        return False
...
def run_compact_update():
    ...
    if not (trading_start <= now_eastern <= trading_end):
        logger.info(
            f"INFO: Outside of all trading hours (4:00 AM - 8:00 PM ET). Current time: {now_eastern.strftime('%H:%M:%S ET')}. Skipping real-time update."
        )
        sys.exit(0)

    logging.info(f"Market hours check PASSED...")
    logger.info(
        f"✅ Within trading hours ({now_eastern.strftime('%H:%M:%S ET')}). Proceeding with real-time update."
    )
    ...
    logger.info(
        f"📋 Processing {len(tickers)} tickers from master_tickerlist.csv: {tickers}"
    )
    logger.info(
        "🔄 This engine will process EVERY single ticker to fix incomplete processing"
    )
    ...
    for i, ticker in enumerate(tickers, 1):
        ...
        logger.info(
            f"📊 {ticker}: {status} (real-time: {'✅' if success_realtime else '❌'})"
        )

        # Rate limiting - respect API limits
        if i < len(tickers):  # Don't sleep after last ticker
            logger.debug(
                f"⏳ Rate limiting: sleeping 0.5 seconds before next ticker..."
            )
    ...
    operation_success_rate = (
        (success_count / total_operations) * 100 if total_operations else 0
    )
    ticker_success_rate = (complete_success / len(tickers)) * 100 if tickers else 0

    logger.info(
        f"📈 Operation success rate: {operation_success_rate:.1f}% ({success_count}/{total_operations})"
    )
    logger.info(
        f"🎯 Ticker success rate: {ticker_success_rate:.1f}% ({complete_success}/{len(tickers)})"
    )
