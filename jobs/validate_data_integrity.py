...
class DataIntegrityValidator:
    ...
    def validate_timestamp_format(self, df: pd.DataFrame, file_path: str) -> List[str]:
        ...
        if not timestamp_col:
            issues.append(
                f"No timestamp column found (expected: timestamp, Date, or datetime)"
            )
            return issues
        ...
        try:
            ...
            for i, ts in enumerate(sample_timestamps):
                ...
                if not ts_str.endswith("+00:00"):
                    issues.append(
                        f"Non-UTC timestamp format: {ts_str} (should end with +00:00)"
                    )

                # Check for suspicious microsecond precision patterns
                if ".970407" in ts_str:
                    issues.append(
                        f"Suspicious artificial microsecond pattern detected: {ts_str}"
                    )
                ...
                try:
                    parsed = pd.to_datetime(
                        ts, utc=True
                    )  # Parse as UTC to avoid timezone comparison issues
                    ...
                    issues.append(
                        f"Invalid timestamp format: {ts_str} (parse error: {e})"
                    )
        ...
        for col in critical_columns:
            if col in df.columns:
                ...
                if col in ["open", "high", "low", "close"]:
                    ...
                    if invalid_prices > 0:
                        issues.append(
                            f"Column '{col}' has {invalid_prices} zero or negative price values"
                        )
                ...
                if col == "volume":
                    ...
                    if zero_volume > 0:
                        issues.append(
                            f"Column 'volume' has {zero_volume} zero volume entries"
                        )
            else:
                    ...
                    for i, price in enumerate(sample_prices):
                        ...
                        if "." in price_str:
                            ...
                            if len(decimal_part) > 8:
                                issues.append(
                                    f"Excessive price precision in {col}: {price_str} ({len(decimal_part)} decimal places)"
                                )

                        # Check for specific artificial patterns we've seen before
                        if (
                            "99.88885801779249" in price_str
                            or "99.56018769783935" in price_str
                        ):
                            issues.append(
                                f"Known artificial price pattern detected in {col}: {price_str}"
                            )
        ...
        try:
            ...
            for col in price_columns:
                if col in df.columns:
                    ...
                    if unique_values == 1 and len(df) > 10:
                        issues.append(
                            f"All values in {col} are identical - possible artificial data"
                        )
                    ...
                    if col == "high" and "low" in df.columns:
                        ...
                        if invalid_highs > 0:
                            issues.append(
                                f"Found {invalid_highs} rows where high < low (impossible)"
                            )
            ...
            if "volume" in df.columns:
                ...
                if unique_volumes == 1 and len(df) > 10:
                    issues.append(
                        "All volume values are identical - possible artificial data"
                    )
    ...
    def scan_ticker_data(self, ticker: str) -> Dict:
        ...
        for file_path in expected_files:
            ...
            if result["status"] == "valid":
                ...
                logger.warning(
                    f"⚠️ {file_path}: CORRUPTED ({len(result['issues'])} issues)"
                )
    ...
    def generate_summary_report(self, ticker_results: List[Dict]) -> str:
        ...
        report.append("-" * 40)
        report.append(
            f"Total files scanned: {self.validation_results['scanned_files']}"
        )
        ...
        report.append(
            f"Missing/Error files: {self.validation_results['missing_files']}"
        )
        ...
        if self.validation_results["scanned_files"] > 0:
            valid_pct = (
                ...
                / self.validation_results["scanned_files"]
            ) * 100
            corrupted_pct = (
                self.validation_results["corrupted_files"]
                / self.validation_results["scanned_files"]
            ) * 100
            missing_pct = (
                self.validation_results["missing_files"]
                / self.validation_results["scanned_files"]
            ) * 100

            report.append(
                f"Valid: {valid_pct:.1f}%, Corrupted: {corrupted_pct:.1f}%, Missing: {missing_pct:.1f}%"
            )
        ...
        for ticker_result in ticker_results:
            ...
            status_icon = (
                ...
                else "⚠️" if corrupted > 0 else "❌"
            )
            report.append(
                f"{status_icon} {ticker}: {valid}/{total} valid, {corrupted} corrupted, {missing} missing"
            )
