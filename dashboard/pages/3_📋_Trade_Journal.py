...
st.write(
    "Log your executions, add notes, and track outcomes. Discipline is the bridge between goals and accomplishment."
)
...
with st.form("trade_entry_form", clear_on_submit=True):
    ...
    with c1:
        trade_ticker = st.selectbox(
            "Ticker",
            options=tickers if tickers else ["Enter Ticker Manually"],
            index=None,
        )
    ...
    with c3:
        trade_outcome = st.radio(
            "Outcome", ["✅ Win", "❌ Loss", "⚪️ Break-Even"], horizontal=True
        )
    ...
    if submitted:
        if not all([trade_ticker, trade_entry > 0, trade_exit > 0]):
            st.warning(
                "Please fill in at least the Ticker, Entry Price, and Exit Price."
            )
        else:
                ...
                [
                    {
                        ...
                        "entry": trade_entry,
                        "exit": trade_exit,
                        "outcome": trade_outcome.split(" ")[
                            1
                        ],  # Store just "Win", "Loss", "Break-Even"
...
if not journal_df.empty:
    ...
    st.info(
        "Your trade journal is empty. Log your first trade using the form above to get started."
    )
