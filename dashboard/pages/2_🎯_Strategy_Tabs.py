...
st.write(
    "This section shows the raw, unfiltered output for each screener job. Each tab represents a specific signal file found in your cloud storage."
)
st.markdown("---")
...
if not signal_files:
    st.info(
        "No screener signal files were found in the `data/signals/` directory in your cloud storage."
    )
    st.write(
        "Once your screener jobs run successfully, their results will appear here automatically."
    )
else:
    # Create clean names for tabs, e.g., "gapgo_signals.csv" -> "Gapgo"
    tab_names = [
        s.replace("_signals.csv", "").replace("_", " ").capitalize()
    ...
    for i, tab in enumerate(screener_tabs):
        with tab:
            ...
            if not screener_df.empty:
                ...
                st.info(
                    f"The {tab_names[i]} screener has not generated any signals yet, or the signal file is empty."
                )
