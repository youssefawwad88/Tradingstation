...
def load_all_signals():
    ...
    cols_to_show = [
        ...
        'target_2r', 'target_3r', 'risk_per_share', 'timestamp'
    ]
...
st.write("All trading signals from all active screeners, consolidated in one view. Data is refreshed automatically.")
...
if not master_signals_df.empty:
    ...
    if selected_direction != 'All':
        filtered_df = filtered_df[filtered_df['direction'].str.lower() == selected_direction.lower()]
...
if not filtered_df.empty:
    ...
    st.caption(f"Displaying {len(filtered_df)} of {len(master_signals_df)} total signals.")
else:
    st.info("No trading signals found. This could be because the market is closed, or no setups have met the criteria yet.")
    st.write("Check the `Scheduler Monitor` to ensure the screener jobs are running successfully.")
