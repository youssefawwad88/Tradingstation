...
try:
    ...
    st.error("Fatal Error: Could not import helper functions from `utils.helpers`. The app cannot function without them.")
    st.stop()
...
with st.expander("Master Configuration", expanded=True):
    ...
    if st.button("💾 Save Full Configuration", use_container_width=True, type="primary"):
        ...
        if list_saved and config_saved:
            ...
            st.error("Failed to save one or more configurations. Check the application logs.")


# --- Raw Data Viewer ---
st.markdown("---")
with st.expander("🔬 Raw Data Viewer", expanded=False):
    st.write("Select a ticker to view the latest raw daily and intraday data stored in the cloud.")

    if not current_tickers:
        st.warning("Cannot display data viewer because the ticker list is empty or could not be loaded.")
    else:
