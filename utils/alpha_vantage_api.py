...
def _make_api_request(params):
    ...
    if not API_KEY:
        print(
            "WARNING: ALPHA_VANTAGE_API_KEY environment variable not set. API calls will be skipped."
        )
        return None
    ...
    try:
        ...
        print(
            f"ERROR: API request timed out after {REQUEST_TIMEOUT} seconds for symbol: {params.get('symbol')}"
        )
        return None
        ...
        print(
            f"An unexpected error occurred during API request for {params.get('symbol')}: {e}"
        )
        return None
...
def get_intraday_data(symbol, interval="1min", outputsize="compact"):
    ...
    if response:
        try:
            ...
            logger.info(
                f"📊 Raw intraday data fetched for {symbol} ({interval}): {len(df)} rows"
            )
            ...
            logger.info(
                f"✅ Intraday data standardized for {symbol} ({interval}): {len(df)} rows with UTC timestamps"
            )
            return df
        except Exception as e:
