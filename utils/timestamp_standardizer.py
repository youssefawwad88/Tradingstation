...
def standardize_timestamp_column(
    ...
    try:
        ...
        if invalid_timestamps > 0:
            logger.warning(
                f"⚠️  Removing {invalid_timestamps} rows with invalid timestamps"
            )
        ...
        if df_copy[timestamp_col].dt.tz is None:
            ...
            logger.info(
                f"   Converting from {df_copy[timestamp_col].dt.tz} to America/New_York"
            )
        ...
        logger.info(
            f"✅ TIMESTAMP STANDARDIZATION COMPLETE: {len(df_copy)} rows processed"
        )
        logger.info(
            f"   Sample standardized timestamp: {df_copy[timestamp_col].iloc[0]}"
        )
...
def standardize_daily_timestamps(
    ...
    try:
        ...
        df_copy[date_col] = df_copy[date_col] + pd.Timedelta(
            hours=16
        )  # Add 16 hours for 4:00 PM
        ...
        logger.info(
            f"✅ DAILY TIMESTAMP STANDARDIZATION COMPLETE: {len(df_copy)} rows processed"
        )
...
def validate_timestamp_standardization(
    df: pd.DataFrame, timestamp_col: str = "timestamp"
) -> bool:
    """
...
def apply_timestamp_standardization_to_api_data(
    df: pd.DataFrame, data_type: str = "intraday"
) -> pd.DataFrame:
    """
...
def convert_stored_data_to_utc(
    df: pd.DataFrame, timestamp_col: str = "timestamp"
) -> pd.DataFrame:
    """
