...
def run_gapgo_screener():
    ...
    for ticker in tickers:
        try:
            ...
            premarket_high = premarket_df['high'].max() if not premarket_df.empty else np.nan
            premarket_low = premarket_df['low'].min() if not premarket_df.empty else np.nan
            ...
            if session == 'REGULAR':
                opening_candle = today_intraday_df.loc[today_intraday_df.index.time == time(9, 30)]
            ...
            live_gap_percent_pm = ((last_price - prev_close) / prev_close) * 100 if prev_close is not None else np.nan
            ...
            # Long conditions
            gap_valid_long = not np.isnan(official_gap_percent) and official_gap_percent >= 1.5
            vwap_reclaimed = not np.isnan(last_vwap) and last_price > last_vwap
            breakout_above = not np.isnan(premarket_high) and last_price > premarket_high
            breakout_candles = today_intraday_df[today_intraday_df['close'] > premarket_high] if not np.isnan(premarket_high) else pd.DataFrame()
            ...
            # Short conditions
            gap_valid_short = not np.isnan(official_gap_percent) and official_gap_percent <= -1.5
            ...
            breakdown_candles = today_intraday_df[today_intraday_df['close'] < premarket_low] if not np.isnan(premarket_low) else pd.DataFrame()
            ...
            if setup_valid and session == 'REGULAR':
                orb_5min_candles = today_intraday_df.between_time(time(9, 30), time(9, 34))
                if not orb_5min_candles.empty:
                    ...
                    breakout_candle_time = first_breakout_time if direction == "Long" else first_breakdown_time
                    breakout_candle = today_intraday_df.loc[breakout_candle_time]
                    bo_range = breakout_candle['high'] - breakout_candle['low']
                    if bo_range > 0:
                        breakout_body_pct = (abs(breakout_candle['close'] - breakout_candle['open']) / bo_range) * 100
                    if avg_orb_vol > 0:
                        breakout_vol_spike_pct = (breakout_candle['volume'] / avg_orb_vol) * 100
