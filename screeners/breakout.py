...
def run_breakout_screener():
    ...
    if (
        ...
        logger.warning(
            "AVWAP anchors file not found or invalid format. AVWAP confluence will not be calculated."
        )
    ...
    for ticker in tqdm(tickers, desc="Scanning for Breakouts"):
        try:
            ...
            if (
                daily_df is None or len(daily_df) < 20
            ...
            daily_df["Avg_Vol_20D"] = (
                daily_df["volume"].shift(1).rolling(window=20).mean()
            )
            daily_df["Volume_vs_Avg_Pct"] = (
                daily_df["volume"] / daily_df["Avg_Vol_20D"]
            ) * 100
            ...
            if pd.notna(latest["BB_Upper"]):
                near_breakout = (
                    "Yes" if latest["close"] > latest["BB_Upper"] * 0.98 else "No"
                )
            if pd.notna(latest["BB_Lower"]):
                near_breakdown = (
                    "Yes" if latest["close"] < latest["BB_Lower"] * 1.02 else "No"
                )
            ...
            if candle_range > 0:
                body_percent = (
                    abs(latest["close"] - latest["open"]) / candle_range
                ) * 100
            ...
            if len(daily_df) >= 6:
                if direction == "Long":
                    ...
                    if (
                        len(prior_5_highs) == 5
                        and latest["close"] > prior_5_highs.max()
                    ):
            ...
            signal = {
                ...
                "Volume vs Avg %": (
                    f"{latest['Volume_vs_Avg_Pct']:.1f}"
                    if pd.notna(latest["Volume_vs_Avg_Pct"])
                    else "N/A"
                ),
